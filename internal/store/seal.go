...
const (
	masterKeySize                   = 32
	EnvAllowMasterKeyInDataRoot     = "NOCTAXRIS_AZ_ALLOW_MASTER_KEY_IN_DATA_ROOT"
)
...
func ResolveMasterKeyPath(configured, dataRoot string) (string, error) {
	...
	if under && !allow {
		return "", fmt.Errorf("master key path %q is under NOCTAXRIS_AZ_DATA_ROOT %q; set NOCTAXRIS_AZ_MASTER_KEY_FILE outside the data root, or %s=1 to allow colocation",
