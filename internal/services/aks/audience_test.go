...
func TestAKSListRejectsGraphAudience(t *testing.T) {
	st := openStore(t)
	defer st.Close()
	...
	h.Register(mux)
	list := "/subscriptions/sub/resourceGroups/rg/providers/Microsoft.ContainerService/managedClusters"
	...
	if drec.Code != http.StatusForbidden {
		t.Fatalf("Graph aud on AKS list %d body=%s", drec.Code, drec.Body.String())
	}
	...
	if orec.Code != http.StatusOK {
		t.Fatalf("root AKS list %d body=%s", orec.Code, orec.Body.String())
	}
}
