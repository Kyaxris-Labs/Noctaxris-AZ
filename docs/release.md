...
How to cut a public Noctaxris-AZ release (example: **1.0.0**). Docker Hub image: **`kyaxris/noctaxris-az`**.
...
git tag -a v1.0.0 -m "Release Noctaxris-AZ 1.0.0"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris-az:1.0.0` | Exact semver |
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.0.0 -t kyaxris/noctaxris-az:local .
