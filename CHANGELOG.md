...
- Event Hubs enqueue dual-writes a capture table. `GET /eventhubs/{ns}/hubs/{hub}/capturedEvents` (and `.../{id}`) is readable by root or Event Hubs Data Receiver / Data Owner (`receive/action`). Subscription Reader is denied. HTTP send/receive stay root Bearer. Event Hubs Data Receiver uses Azure GUID `a638d3c7-ab3a-418d-83e6-5f17a39d4fde` (previous lab GUID still matches).
